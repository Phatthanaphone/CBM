  ...
  const navigate = useNavigate();
    ...
    const handleSubmit = (event) => {
      ...
      if (data.id === "") {
        ...
        ManualService.update(initID.id,data).then((response) => {
    ...
    const fetch = async () => {
        const result = await ManualService.getByID(initID.id)
        setID(result.ID)
        console.log(id)
     }
    useEffect( () => {
    ...
    return (
      ...
      >
         <ArrowLeftIcon style={{marginRight :'0.6rem'}} className="ml-2 -mr-1 h-5 w-5" aria-hidden="true" />
